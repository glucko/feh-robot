...
DEVELOPER_FIRMWARE_PATH = "D:\dev\erc2firmware" # Path to your local firmware repository
...
def updateFirmware():
    ...
    if DEVELOPER_MODE:
        ...
        shutil.copytree(DEVELOPER_FIRMWARE_PATH, lib_path, dirs_exist_ok=True, ignore=shutil.ignore_patterns('*.git', 'README.md', 'LICENSE'))
    else:
        # If on Windows, check if on a Hitchcock computer

        if IS_WINDOWS:
            git_in_path = os.system("git status >nul 2>&1") == 0
            if not git_in_path:
        ...
        command += f'{GIT_EXE} submodule status {LIBRARY_SUBMODULE} || (' + \
            ...
            f'){COMMAND_DELIMITER}' 
        command += f'{GIT_EXE} submodule update --init --recursive --checkout --force --remote'
        os.system(command)
